...
function loadImages() {
  let url = `dbModule/searchImage.php?order=${encodeURIComponent(order)}
    ...
    limit
  )}&current_page=${encodeURIComponent(
    current_page
  ...
  if (search_form.getAttribute("saved") ) {
    let saved = search_form.getAttribute("saved");
    url += `&saved=${encodeURIComponent(saved)}`;
  }

  if(search_form.getAttribute("user_id")){
...
function adjustGridDisplay() {
  ...
  } else if (search_form.getAttribute("removable")) {
    makeImagesRemovable();
  } else {
    makeImagesClickable(); 
  }
...
function makeImagesRemovable() {
  ...
  Array.from(images).forEach((image) => {
    ...
    removeIcon.classList.add("fas", "fa-x", "remove-button");
    image.appendChild(removeIcon);

    let image_element = image.getElementsByTagName('img')[0];
    let image_id = image_element.id.replace('image','');
    let album_id = search_form.getAttribute('album_id'); 
    console.log(image_id, album_id)

    removeIcon.addEventListener("click", () => {
      const url = "dbModule/removeImageFromAlbum.php";
